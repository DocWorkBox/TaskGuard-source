...
- 数字使用等宽数字，避免 `0.0%` 到 `100.0%` 切换时造成行内抖动。
- 标题栏总占用块右对齐，建议宽度 112-132 px，显示两行：`CPU x.x%`、`内存 x.x%`。
- 服务组行占用块右对齐，建议宽度 78-96 px，显示两行：CPU、内存。
- 进程行占用块放在命令行区域右侧，端口信息之前；如果宽度不足，优先截断命令行和 cwd，不隐藏 CPU/内存。
- CPU 达到高占用阈值时使用橙色强调；普通状态使用次级文本色。
- 资源占用刷新只替换数字，不改变行高、列宽、排序和选中状态。
- 快速面板、主窗口服务列表、详情进程树使用同一套格式化规则，避免不同视图显示不一致。
